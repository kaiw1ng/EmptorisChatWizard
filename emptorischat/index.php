	...
	$("#btnEnter").click(function()
{
	if ($("#Question").val() !== "")
	{
		...
		$.post('ServiceCall.php', FormData, function(JSONResponse) 
		{
			...
			{
				$("#Agreement_Type").val("9270d3a59c3743b49b6d3205ff7045ee");//Hard coding for the demo
			}
			else if (Intent === 'int_agreement_primext_party')
			{
				var DivHtml= "";
				$('#Preloader').modal( { show: true, backdrop: 'static', keyboard: false, opacity:1 });
				$.post('SoapClientSuppliers.php', {SupplierName:Question}, function(JSONResponseSuppliers)
				{
					...
					else if (TotalResponses === 0)
					{
						...
						Response+= '<br /> <a href="https://sandbox.ssmcloud.com/srm-app/ux/selfreg.html#/self-reg/">https://sandbox.ssmcloud.com/srm-app/ux/selfreg.html#/self-reg/</a>'
			...
			else if (Intent === 'int_agreement_scope')
			{
				...
				$.post('SoapClientContracts.php', FormData, function(JsonRecord) 
				{
						var QDT			= new Date(Date.now()).toString();
						var	fQDT		= QDT.substring(4, QDT.indexOf('GMT') );
						var ContractURL	= "https://sandbox.ssmcloud.com/dicarta/servereq?hname=cs_load_cs&summary=true&contractID="+JsonRecord['ns2:id'];
						var Response	= 'Congratulations, a new contract '+ JsonRecord['ns2:contract-number'] +' has been created as per the details provided. You may directly access this contract by clicking <a href="'+ContractURL+'" action="_blank">here</a>';
						var MessageResp	= '<div class="container-fluid" style="width:97%">';
							MessageResp+= '	<div class="row">';
							MessageResp+= '		<div class="col-xs-6">&nbsp;</div>';
							MessageResp+= '		<div class="col-xs-6 ChatResponse"><strong>Watson:&nbsp;&nbsp;</strong>'+Response+'</div>';
